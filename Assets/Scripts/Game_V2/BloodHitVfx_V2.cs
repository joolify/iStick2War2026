...
{
    /*
 * BloodHitVfx_V2 (Static blood / hit chunk spawner)
 *
 * PURPOSE:
 * Instantiates a configured prefab at a world hit point, rotates toward incoming shot travel, scales with damage vs
 * reference damage, and applies high sorting order so splats render above Spine sprites. Optional timed Destroy.
 *
 * ---------------------------------------------------------
 * ❌ MUST NOT
 *
 ...
 */
    ...
    {
        // Draw on top of typical Spine / sprite layers so hits on the body are not fully occluded.
        private const int ForegroundSortOrder = 8000;

        // World-space direction the projectile traveled (into the target).
        // Scales instance size vs .
        ...
        // Cancels prefab Play On Awake emission so we can configure modules, then Play once.
        ...
        // Aligns local +X with dir in the XY plane (2D shooter convention).
