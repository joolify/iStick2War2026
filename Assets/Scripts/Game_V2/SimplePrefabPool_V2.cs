...
{
    /*
 * SimplePrefabPool_V2 (Static prefab pool)
 *
 * PURPOSE:
 * Tracks inactive stacks per prefab key for long Play Mode sessions: Spawn reuses or instantiates, Despawn pushes
 * back onto the stack with optional reset hooks. Falls back to Destroy when the instance was never pooled.
 *
 * ---------------------------------------------------------
 * ❌ MUST NOT
 *
 * - Reference scene-specific singletons (static utility only).
