 ...
 * INPUT SOURCES
 *
 * - WaveManager_V2 (starts / stops wave spawn routines, supplies WaveConfig_V2-driven parameters).
 * - Scene references: prefabs, spawn anchors, camera for frustum-aligned spawns.
 *
 * ---------------------------------------------------------
 * ❌ MUST NOT
 *
 * - Own shop economy, currency, or between-wave UI (WaveManager_V2 / ShopPanel_V2).
 ...
 */
    public sealed class EnemySpawner_V2 : MonoBehaviour
    {
        ...
        // 1-based wave index from WaveManager_V2.CurrentWaveNumber for debug logs; 0 if unset.
        ...
        // (matches the spawn half of ). While the wave is still active, long gaps without
        ...
        // One-line spawner state for GameError / telemetry (call before StopWave if possible).
            ...
            Action<GameObject, bool> onSpawned)
        {
            ...
            // Pool returns with SetActive(true), but prefab/children may still be inactive; match Paratrooper/Mech path.
        ...
        // Indented line for multi-line debug blocks (mount sample, spawn snapshot, drop decision).
        ...
        // Single-line suffix for short log messages.
        ...
        // Spawns just past the orthographic frustum on X; Y from .
        ...
        // Forces horizontal spawn to match EnemySpawner invert flags so bombplanes always
        ...
        private void ApplyAircraftSpriteSorting(GameObject aircraft)
        {
            ...
            // Sprites (helicopter / bomber) and Spine mesh renderers (kamikaze / bomb drone) all respect 2D sorting.
            SpriteRenderer[] spriteRenderers = aircraft.GetComponentsInChildren<SpriteRenderer>(true);
            ...
            MeshRenderer[] meshRenderers = aircraft.GetComponentsInChildren<MeshRenderer>(true);
            for (int i = 0; i < meshRenderers.Length; i++)
            {
                ...
                if (layerId.HasValue)
                {
                    mr.sortingLayerID = layerId.Value;
                }

                mr.sortingOrder = _aircraftSpriteSortingOrder;
            }
        }
