...
{
    /*
 * BunkerDamageTelemetrySource (Telemetry attribution bucket)
 *
 * PURPOSE:
 * Tags bunker HP loss rows in WaveRunTelemetry_V2 JSON so analysts can split bomb splash from generic enemy fire.
 * Other values are reserved for forward-compatible expansion.
 *
 * ---------------------------------------------------------
 * DESIGN PRINCIPLE
 *
 ...
 */
    ...
    {
        Other = 0,
        // HP absorbed from BombProjectile_V2 explosions (distinct from generic Other when attributed).
        Bomb = 1
    }

    /*
 * WaveRunTelemetry_V2 (Session JSON telemetry)
 *
 * PURPOSE:
 * Writes one JSON document per session with an "events" array (pretty-printed). Tracks combat counters,
 * bunker HP samples and pressure metrics, shop spend attributed to the wave that just ended, optional Unity
 * console capture, and feel-KPI rows. session_begin is deferred one frame after enable so WaveManager_V2
 * economy and bunker values have initialized.
 *
 * ---------------------------------------------------------
 * INPUT SOURCES
 *
 * - WaveManager_V2 (state transitions, bunker damage, shop spend, wave boundaries).
 * - Hero_V2 when present (per-wave damage and healing aggregates).
 *
 * ---------------------------------------------------------
 * ❌ MUST NOT
 *
 ...
 * DESIGN PRINCIPLE
 *
 * Append-only telemetry sink: subscribe early, flush to disk on session end; intended for offline balance and QA tools.
 */
    public sealed class WaveRunTelemetry_V2 : MonoBehaviour
    {
        ...
        {
            // Seconds since this wave entered InWave (matches TelemetryEvent.waveDurationSec basis).
        ...
        {
            // Human-readable overview; safe for consumers to ignore when parsing events.
            public string _comment;
            public TelemetryGlossary glossary;

            // Clamped fraction used for TelemetryEvent.bunkerCriticalLow on each event in this file.
            ...
            // Clamped ratio threshold for TelemetryEvent.bunkerPressureTimeSec (Inspector echo).
            public float bunkerPressureHpRatioThresholdUsed;

            public TelemetryEvent[] events;

            // Optional: Unity console errors/exceptions with a gameplay snapshot (same sessionId as events[]).
            public TelemetryUnityLogRow[] unityLogs;

            // Optional: design/feel proxies (milestones + deltas vs session_begin realtime).
            public TelemetryFeelSessionSummary feelSession;

            // Optional: prefab pool counters snapshot (SimplePrefabPool_V2) at write time.
        ...
        {
            // Time.realtimeSinceStartup when session_begin row was written; -1 if unknown.
            ...
            // Seconds after session_begin; -1 if milestone never occurred.
        ...
        private sealed class TelemetryEvent
        {
            ...
            // Subset of damageTakenBunker from bomb explosions (BombProjectile_V2).
            ...
            public string weaponType;
            // on the hero when present; empty if no bot / disabled.
            public string autoHeroTestProfile;
            public bool autoHeroHasTarget;
            public bool autoHeroInRange;
            public bool autoHeroCanHoldFire;
            ...
            public int autoHeroFallbackLivingParatrooperModels;
            public int autoHeroFallbackEnabledEnemyBodyPartColliders;
            // True if bot had a combat target on at least one InWave frame for this wave (see glossary).
            public bool autoHeroInWaveAnyHadTarget;
            public string autoHeroInWaveLastTargetKindWhenHadTarget;
            public string autoHeroInWaveLastTargetParatrooperStateWhenHadTarget;
            ...
            // True when bunkerHp snapshot is <= 0 (cover breached).
            ...
            // True when hero is dead at snapshot (see run_end / session_quit).
            public bool heroDead;

            // Rough burst-ish scalar: meaningful on wave_cleared and on run_end when GameOver occurs during
            ...
            // Non-empty only on wave_cleared / run_end (abort): JSON from TelemetryWaveScaling via JsonUtility.
        ...
        // Called from WaveManager_V2 when bunker loses HP.
        public static void NotifyBunkerDamageTaken(int amount)
        {
            NotifyBunkerDamageTaken(amount, BunkerDamageTelemetrySource.Other);
        }

        // Called from WaveManager_V2.ApplyBunkerDamage with a coarse damage source.
        ...
        // Called from WaveManager_V2 after a successful shop spend.
        ...
        // Call from WaveManager_V2.ReportEnemyKilled for feel-KPI first-kill timing.
        public static void NotifyEnemyKilledForFeelKpis()
        {
            ActiveInstance?.RegisterFirstEnemyKillRealtime();
        }

        // Call when a combat stun/EMP is applied to an enemy during InWave.
        public static void NotifyEmpCombatStunApplied()
        {
            ActiveInstance?.RegisterEmpCombatStunApplied();
        }

        // Record a non-Unity exception signal (watchdog, sanity check, etc.) into unityLogs[] with the same
        ...
        // Single scalar for sorting/graphing waves; not a game-design "difficulty tier".
