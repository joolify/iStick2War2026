...
{
    /*
 * WaveRunTelemetryAnalyzerWindow (Editor: telemetry JSON heuristics)
 *
 * PURPOSE:
 * Unity EditorWindow that loads WaveRunTelemetry_V2 JSON (JsonUtility-compatible), scans event rows, and surfaces
 * v1 heuristic flags (chain collapse, pressure spikes, etc.) for designers without custom spreadsheet tooling.
 *
 * ---------------------------------------------------------
 * ❌ MUST NOT
 *
 ...
 */
    ...
    {
        // End state of prior wave_cleared/run_end for COLLAPSE_CHAIN (file order).
        ...
        // Min total bunker damage before BOMB_DOMINATED_BUNKER (avoid trivial rows).
        private const int BombDominatedBunkerMinTotalDamage = 20;

        // Bomb bunker subset must exceed this fraction of total bunker damage (strict >).
        private const float BombDominatedBunkerRatioThreshold = 0.9f;

        // Swedish v1 heuristic legend shown at top of report and in UI before first analysis.
        ...
        // Field guide for JSON + batch columns (Swedish); appended to Analyze/Batch reports and optional UI foldout.
        ...
        // When true, skip OBJECTIVE_INACTIVE, TRIVIAL_BUNKER, and SWEET_SPOT_HINT for early waves (onboarding).
        ...
        // Mean of bunker/hero start ratios (same row max HP as proxy).
        ...
        // Time-weighted mean (1 − bunkerHpRatio) from samples; complements threshold-based pressureScore.
        ...
        // Share of bunkerHpSamples strictly below pressure ratio threshold (discrete continuity proxy).
        ...
        // Sustained bunker pressure vs wave length (0–1).
        ...
        // Weapon / enemy interaction only (no bunker damage).
        ...
        // Incoming threat to hero + bunker objective (damage rates and lowest cover depth).
        ...
        // Fail / collapse: terminal state → 1; else depth from minRatio + bunker ratio slope (0–1).
        ...
        // Hero damage taken per second above this counts the whole wave duration toward "overwhelmed" time.
        ...
        // Linear slope of bunkerHpRatio over sampled wave time (rough collapse speed).
